...
// File: @rareswap/rareswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/RareSwapProfile.sol

pragma solidity ^0.6.0;

/** @title RareSwapProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract RareSwapProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public rareToken;
    ...
    uint256 public numberRareToReactivate;
    uint256 public numberRareToRegister;
    uint256 public numberRareToUpdate;
    ...
    constructor(
        IBEP20 _rareToken,
        uint256 _numberRareToReactivate,
        uint256 _numberRareToRegister,
        uint256 _numberRareToUpdate
    ) public {
        rareToken = _rareToken;
        numberRareToReactivate = _numberRareToReactivate;
        numberRareToRegister = _numberRareToRegister;
        numberRareToUpdate = _numberRareToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends RARE to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        rareToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberRareToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer RARE token to this address
        rareToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberRareToUpdate
    ...
    /**
     * @dev Claim RARE to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        rareToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of RARE to register
     * Callable only by owner admins.
     */
    function updateNumberRare(
        uint256 newNumberRareToReactivate,
        uint256 _newNumberRareToRegister,
        uint256 _newNumberRareToUpdate
    ) external onlyOwner {
        numberRareToReactivate = newNumberRareToReactivate;
        numberRareToRegister = _newNumberRareToRegister;
        numberRareToUpdate = _newNumberRareToUpdate;
    }

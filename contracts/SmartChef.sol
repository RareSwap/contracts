...
// added functionnalities by the rareswap's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when RARE mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rareReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake RARE tokens to TheBushV1
    ...
    // Withdraw RARE tokens from STAKING.
